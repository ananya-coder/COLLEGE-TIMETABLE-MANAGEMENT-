...
<!--NAVBAR SECTION END-->
<br>
<form action="allotpracform.php" method="post" style="margin-top: 100px">
    <div align="center">
...
<table id=allotedpracticalstable>
    ...
    while ($row = mysqli_fetch_assoc($q)) {
        ...
        $t1 = mysqli_query(mysqli_connect("localhost", "ananya", "ananya1234", "timetable"),
            "SELECT faculty_name FROM teachers WHERE faculty_id = '$teacher_id1'");
        $trow1 = mysqli_fetch_assoc($t1);
       
        echo "<tr><td>{$row['subject_code']}</td>
                    <td>{$row['subject_name']}</td>
                    <td>{$row['allotedto']}</td>
                    <td>{$trow1['faculty_name']}</td>
                    <td>
