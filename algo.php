...
class Subject
{
    public $subject_code; //Subject code
    public $classes = 0; //No. of classes
    public $semester; //semester of subject
    public $faculty_id; //faculty number of teacher1
}

/**Class to store teachers details**/
class Teacher
{
    public $faculty_id; //faculty number
    public $days = array(); //schedule
    public $faculty_name = array(); //classroom names
}

$subjectslots = array(); //subjects slots for all semesters


$query = mysqli_query(mysqli_connect("localhost", "ananya", "ananya1234", "timetable"), "SELECT * FROM subjects ");
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "ananya", "ananya1234", "timetable"),
            "SELECT * FROM teachers WHERE faculty_id='$temp->subjectteacher'");
...
$query = mysqli_query(mysqli_connect("localhost", "ananya", "ananya1234", "timetable"), "SELECT * FROM teachers ");
...
for ($I = 0; $I < $subjects_count * 4; $I++) {
    ...
    $year = ($sem + 1) / 2;
    $classroom_query = mysqli_query(mysqli_connect("localhost", "ananya", "ananya1234", "timetable"),
        ...
        if (isset($subjectslots[$sem][$r % 6][$j % 5])) {
            ...
            for ($z = 0; $z < 5; $z++) {
                if (isset($subjectslots[$sem][$r % 6][$z])) {
                    ...
                    if ($subjectslots[$sem][$r % 6][$z] == $subjects[$i]->subject_code) {
                        $already = true;
                    }
                }
            }
            ...
            // set subject
            $subjects[$i]->classes++;
            $subjectslots[$sem][$r % 6][$j % 5] = $subjects[$i]->subject_code;
            $teachers[$tindex]->days[$sem % 2][$r % 6][$j % 5] = $subjects[$i]->subject_code;
...
period2='" . $subjectslots[$i][$j][1] . "', 
period3='" . $subjectslots[$i][$j][2] . "',
period4='" . $subjectslots[$i][$j][3]  . "', 
period5='" . $subjectslots[$i][$j][4] . "',
period6='" . $subjectslots[$i][$j][5] . ", '
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "ananya", "ananya1234", "timetable"), $query);
    }
